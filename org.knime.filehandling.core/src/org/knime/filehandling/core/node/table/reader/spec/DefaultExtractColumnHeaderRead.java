...
final class DefaultExtractColumnHeaderRead<V> implements ExtractColumnHeaderRead<V> {
    ...
    DefaultExtractColumnHeaderRead(final Read<V> source, final TableReadConfig<?> config) {
        ...
        if (config.useColumnHeaderIdx()) {
            ...
            CheckUtils.checkArgument(m_numRowsToSkip >= 0, "The column header index cannot negative.");
        } else {
            colHeaderIdx = -1;
        ...
        if (config.skipRows()) {
            ...
            CheckUtils.checkArgument(m_numRowsToSkip >= 0, "The number of rows to skip cannot negative.");
        } else {
            m_numRowsToSkip = 0;
        }
        ...
        if (config.limitRowsForSpec()) {
            ...
            CheckUtils.checkArgument(m_numRowsToSkip >= 0, "The number of rows to scan cannot be negative.");
        } else {
            m_numRowsToRead = -1;
        }
        m_read = read;
