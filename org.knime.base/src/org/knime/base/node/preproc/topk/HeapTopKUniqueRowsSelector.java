...
final class HeapTopKUniqueRowsSelector implements TopKSelector {
    ...
    private final PriorityQueue<UniqueRowHelper> m_topRows;
    ...
    public void consume(final DataRow row) {
        final int comparisonResult = m_topRows.isEmpty() ? 1 : m_comparator.compare(m_topRows.element().m_representative, row);

        if (m_topRows.size() < m_nrRows) {
            if (comparisonResult < 0) {
                final Optional<UniqueRowHelper> uniqueRow = searchUniqueRowHelper(row);
                if (uniqueRow.isPresent()) {
                    uniqueRow.get().m_rows.add(row);
                    return;
                } else {
                    m_topRows.add(new UniqueRowHelper(row));
                    return;
                }
     ...
     */
    private Optional<UniqueRowHelper> searchUniqueRowHelper(final DataRow row) {
